...
Alle Server (sowohl Web- wie auch Appserver), schreiben ihre jeweiligen Logfiles auf ein \ac{NAS}. Der Infrastrukturserver mounted die entsprechenden Verzeichnisse. Der Server verfügt über eine Hadoop Installation, auf welcher die Applikation ausgeführt wird (siehe \autoref{fig:AufbauInfrastruktur}).
...
Ein MapReduce Job beginnt mit der Übertragung der Daten an die Klasse \textit{InputFormat}. Diese teilt (\textit{Split}) die Informationen in annähernd gleich große Teile. Dabei werden die Daten mit dem \textit{RecordReader} ausgetauscht, welcher innerhalb eines Splits die initialen Schlüssel-Wert-Paare ermittelt. Im Anschluss werden diese an die \textit{Map}-Funktion weitergegeben. Je nach Größe der Daten werden $n$ Splits parallel verarbeitet.

Erst wenn alle \textit{Mapper} die Verarbeitung abgeschlossen haben, werden die neuen Schlüssel-Wert-Paare partitioniert und sortiert. Diese Sortierten Paare werden dem \textit{Reducer} übergeben.

Abschließend wird das Ergebnis der Analyse der Klasse \textit{OutputFormat} übergeben, welche das Resultat aufbereitet und im \ac{HDFS} ablegt. \autoref{fig:DatenflussMapReduce} stellt den Datenfluss zur Verdeutlichung graphisch dar.
...
\begin{table}
	...
	\begin{tabular}{| r | l | r |}
		...
		\multicolumn{1}{|l|}{\cellcolor[HTML]{4976AA}{\color[HTML]{FFFFFF} {\bf \#}}} & \multicolumn{1}{l|}{\cellcolor[HTML]{4976AA}{\color[HTML]{FFFFFF} {\bf Anzahl Einträge}}} & \multicolumn{1}{l|}{\cellcolor[HTML]{4976AA}{\color[HTML]{FFFFFF} {\bf Durchsch. Laufzeit (in Sekunden)}}} \\ \hline
		1 & $10$ & 7,4 \\  \hline
		2 & $100$ & 7,1 \\ \hline
		3 & $1000$ & 7,3 \\  \hline
		4 & $10^4$ & 9,1 \\  \hline
		5 & $5\times10^4$ & 9,1 \\  \hline
		6 & $2,5\times10^5$ & 11,0 \\  \hline
		7 & $1,5\times10^6$ & 21,7 \\  \hline
		8 & $3\times10^6$ & 33,7 \\  \hline
		9 & $6\times10^6$ & 59,5 \\  \hline
		10 & $10^7$ & 88,9 \\  \hline
	\end{tabular}
